...
RegisterNetEvent("DeadStocks:Buy")
AddEventHandler("DeadStocks:Buy", function(data, cb)
    ...
    if (xPlayer.getMoney() >= costPer) then 
        ...
        if (GetStockCount(src) < GetAllowedCount(src)) then 
            ...
            TriggerClientEvent("DeadStocks:SendNotif", src, "<span class='buy'>SUCCESS: Purchased a stock of " .. stockAbbrev .. "</span>");
            TriggerEvent("DeadStocks:SetupDataID", src);
            --cb('ok');
        else 
            -- They already have the max number of stocks they are allowed 
            TriggerClientEvent("DeadStocks:SendNotif", src, "<span class='error'>ERROR: You already have the max number of stocks you " .. 
                "are allowed to own...</span>");
        end
    else 
        -- They do not have enough money to afford this 
        TriggerClientEvent("DeadStocks:SendNotif", src, "<span class='error'>ERROR: You do not have enough money to afford this...</span>");
    end
end)
...
RegisterNetEvent("DeadStocks:SetupData")
AddEventHandler("DeadStocks:SetupData", function()
    local src = source;
    local data = GetStockPurchaseData(src);
    TriggerClientEvent("DeadStocks:SendData", src, data);
end)
RegisterNetEvent("DeadStocks:SetupDataID")
AddEventHandler("DeadStocks:SetupDataID", function(src)
    local data = GetStockPurchaseData(src);
    TriggerClientEvent("DeadStocks:SendData", src, data);
end)
RegisterNetEvent("DeadStocks:Sell")
AddEventHandler("DeadStocks:Sell", function(data, cb)
    ...
    if HasStockOwned(src, stockAbbrev, 1) then 
        ...
        xPlayer.setMoney(xPlayer.getMoney() + costPer);
        TriggerClientEvent("DeadStocks:SendNotif", src, "<span class='sell'>SUCCESS: Sold a stock of " .. stockAbbrev .. "</span>");
        TriggerEvent("DeadStocks:SetupDataID", src);
        --cb('ok');
    else 
        -- They do not own this stock 
        TriggerClientEvent("DeadStocks:SendNotif", src, "<span class='error'>ERROR: You do not own any of this stock...</span>");
    end 
end)
MySQL.ready(function()
    ...
    RegisterNetEvent('DeadStockMarket:Server:GetMaxStocks')
    AddEventHandler('DeadStockMarket:Server:GetMaxStocks', function()
        local src = source;
        local curAmt = 0;
        ...
        TriggerClientEvent('DeadStockMarket:Client:SetMaxStocksOwned', src, curAmt)
    end)
    RegisterNetEvent('DeadStockMarket:Server:GetStockHTML')
    AddEventHandler('DeadStockMarket:Server:GetStockHTML', function()
        local stockData = {}
        local src = source;
        ...
        TriggerClientEvent('DeadStockMarket:Client:GetStockData', src, stockData);
    end)
